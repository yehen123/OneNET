<template>
	...
	<!--子组件位置-->
	<datatable :condition="conditionArray" :tabledata="tabledata" :subdata="productData" :searchdata.sync="searchdata"></datatable>
	...
	export default{
		data: function () {
			return {
				...
				// 分页数据
				showRightMoreTail: false, // 是否显示右侧省略
				showLeftMoreTail: false, // 是否显示左侧省略
				productData: false,
				total: '',
				size: 20, // 每页数据条数
				dataTotal: '',
		...
		methods: {
			...
			pageJump: function (index) {
				if(parseInt(index) > parseInt(this.$data.total) || parseInt(index) < 0 || !parseInt(index)) {
					...
					paras.page = index;
					var url = "/report/getproductreportajax";
					...
					success: function(res){
						if(res.code == 0) {
							if (type == 1) {
								...
								var pageTotal = Math.ceil(res.data.total / $this.$data.size);

								// 设置分页数据
								if(res.data.items.length != 0 ) {
									$this.$set("productData", res.data.items); // 表格内的数据
								} else {
									$this.$set("productData", '无数据'); // 表格内的数据
								}
								$this.$set("total", pageTotal);  // 总页数
								$this.$set("dataTotal", res.data.total); // 数据总数
							}
						} else {
							console.log(res.msg);
						}
